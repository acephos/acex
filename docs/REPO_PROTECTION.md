...
6. Restrict admin bypass if the repository plan permits it; otherwise record any admin bypass in `docs/checkpoint-ledger.jsonl` as a `process` or `correction` entry.
7. Keep Actions enabled for pull requests and pushes with `fetch-depth: 0` in the ledger workflow.
...
- in-repo checks: validator, workflow prefix check, CODEOWNERS;
- repository settings: required status check, code-owner review, no force pushes.
