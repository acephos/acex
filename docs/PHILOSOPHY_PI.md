...
Status: `acex_discover::scan` is shipped; `--status` / `--smoke` expose summaries. G1 core and recommended G1.5 board/palette/focus/peek/send/start/wait/Zed/attach/worktrees/workspaces are usable; active G2 work is tracker-selected polish, with F35 cached ANSI peek rendering shipped.
