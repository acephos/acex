...
1. **Discover** — `cargo run -p acex -- --help` for flag/usage, `cargo run -p acex -- --checkpoint-status` for continuation state, or `cargo run -p acex -- --status` for live connection/discovery status.
